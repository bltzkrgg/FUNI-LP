...
import { fetchV4UniswapTvlForPools, persistFetchedV4UniswapTvl } from '../../workers/src/v4-tvl-refresh.js';
...
type DirectLookupCandidateRow={pool_id:string;currency0:string;currency1:string;initialize_fee_raw:number;tick_spacing:number;hooks:string;active_liquidity_raw:string|null;validation_status:string;initialized:number;last_refreshed_at:string|null;tvl_usd?:unknown;tvl_source?:unknown;tvl_observed_at_ms?:unknown;tvl_fresh_until_ms?:unknown;tvl_status?:unknown};
...
function settleDirectLookupCandidateFromRegistry(repo:SqliteLedgerRepository,request:DirectTokenLookupRequestRef,poolId:string,now:number){const row=repo.v4RegistryPool(poolId),liquidity=BigInt(String(row?.active_liquidity_raw??0)),trustedTvlUsd=trustedV4PoolUsdMetric(row,now).usd,hasLiquidityEvidence=liquidity>0n||trustedTvlUsd!==null;if(!row)return updateDirectLookupCandidate(repo,request,poolId,'EVIDENCE_UNAVAILABLE','STATEVIEW_EVIDENCE_NOT_PERSISTED',now);if(Number(row.initialized)!==1)return updateDirectLookupCandidate(repo,request,poolId,'EVIDENCE_UNAVAILABLE','FRESH_STATEVIEW_NOT_INITIALIZED',now);return updateDirectLookupCandidate(repo,request,poolId,hasLiquidityEvidence?'ELIGIBLE':'NO_ACTIVE_LIQUIDITY',liquidity>0n?'FRESH_POSITIVE_ACTIVE_LIQUIDITY':trustedTvlUsd!==null?'FRESH_TRUSTED_TVL_LIQUIDITY':'FRESH_ZERO_ACTIVE_LIQUIDITY',now);}
