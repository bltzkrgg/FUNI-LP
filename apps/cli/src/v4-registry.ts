...
import {
  ...
  inspectV4Pool, poolId, readV4InitializeEvents, v4ExecutionBlockers, v4ExecutionStaticFeeCapPips, V4_ROBINHOOD_DEPLOYMENTS, v4StateViewAbi, type V4InitializeEvent, type V4PoolKey,
} from '@funi/v4';
...
export type V4LiquidityEvidence='stateview-active-liquidity'|'trusted-uniswap-tvl'|'none';
export type V4Candidate={
 ...
 liquidityEvidence:V4LiquidityEvidence;
...
function liquidityEvidence(liquidity:bigint,trustedTvlUsd:number|null):V4LiquidityEvidence{
 return liquidity>0n?'stateview-active-liquidity':trustedTvlUsd!==null?'trusted-uniswap-tvl':'none';
}
...
export async function v4PoolsForToken(input:{repo:SqliteLedgerRepository;rpc:FallbackRpc;token:Address;fundingAssets?:Address[];auditDeployment?:boolean;onEvent?:(name:string,data:Record<string,unknown>)=>void}){
 ...
 const inspected=await mapConcurrent<Record<string,unknown>,V4Candidate|null>(rows,6,async row=>{
  ...
  const trustedTvlUsd=trustedV4PoolUsdMetric(row).usd,liquidityProof=liquidityEvidence(state.liquidity,trustedTvlUsd);
  const valuation:V4Valuation=trustedTvlUsd!==null?{status:'available',estimatedTvlUsd:trustedTvlUsd,provenance:'trusted Uniswap LP API TVL'}:{status:'unavailable',reason:'active liquidity is not TVL; complete initialized-tick/position inventory is unavailable',provenance:'StateView active liquidity only'};
...
export function cachedV4PoolsForToken(input:{repo:SqliteLedgerRepository;token:Address;fundingAssets?:Address[];now?:number}){
 ...
 const candidates:V4Candidate[]=rows.map(row=>{
  ...
  const persistedStructural=persisted.filter(x=>x!=='ZERO_ACTIVE_LIQUIDITY'&&x!=='EXTREME_STATIC_FEE'),structural=[...new Set([...persistedStructural,...fee.blockers,...hooks.blockers,...(fee.staticFeePips!==null&&fee.staticFeePips>v4ExecutionStaticFeeCapPips()?['EXTREME_STATIC_FEE']:[])])],unsupported=[...structural.filter(x=>x==='EXTREME_STATIC_FEE'),...structural.filter(x=>x==='NONZERO_HOOK_UNSUPPORTED'),...structural.filter(x=>x==='DYNAMIC_FEE_UNSUPPORTED'),...structural.filter(x=>x!=='EXTREME_STATIC_FEE'&&x!=='NONZERO_HOOK_UNSUPPORTED'&&x!=='DYNAMIC_FEE_UNSUPPORTED')],initialized=Number(row.initialized)===1,stateFresh=cacheAgeMs!==null&&cacheAgeMs<=120_000;
  const liquidity=BigInt(String(row.active_liquidity_raw??0)),trustedTvlUsd=trustedV4PoolUsdMetric(row,now).usd,liquidityProof=liquidityEvidence(liquidity,trustedTvlUsd),executionEligible=unsupported.length===0&&initialized&&stateFresh&&liquidityProof!=='none',uiState:V4Candidate['uiState']=unsupported.length?`UNSUPPORTED:${unsupported[0]}`:!initialized?'TEMPORARILY_UNAVAILABLE':!stateFresh?'CHECKING':liquidityProof==='none'?'SUPPORTED_NO_ACTIVE_LIQUIDITY':'EXECUTABLE';
  const valuation:V4Valuation=trustedTvlUsd!==null?{status:'available',estimatedTvlUsd:trustedTvlUsd,provenance:'trusted Uniswap LP API TVL'}:{status:'unavailable',reason:'No fresh StateView active-liquidity or trusted Uniswap LP API TVL evidence',provenance:'StateView + Uniswap LP API'};
  return {protocolVersion:'v4' as const,poolId:String(row.pool_id),key,target,funding:{address:fundingAddress,symbol:String(fundingRow?.symbol??(fundingAddress.toLowerCase()===robinhoodMainnet.assets.USDG.toLowerCase()?'USDG':fundingAddress.toLowerCase()===robinhoodMainnet.assets.WETH.toLowerCase()?'WETH':`${fundingAddress.slice(0,6)}…`)),name:String(fundingRow?.name??'Cached funding token'),decimals:Number(fundingRow?.decimals??(fundingAddress.toLowerCase()===robinhoodMainnet.assets.USDG.toLowerCase()?6:18)),canonical:true},targetIndex,fundingIndex,priceFundingPerTarget:null,priceProvenance:'persisted StateView cache',liquidity,feeLabel:feeLabel(fee),tickSpacing:key.tickSpacing,trustedTvlUsd,liquidityEvidence:liquidityProof,feeSemantics:fee,hookStatus:hooks,valuation,refreshBlock:BigInt(String(row.refresh_block??0)),executionEligible,blockers:unsupported,initializationBlock:BigInt(String(row.initialization_block)),cacheAgeMs,uiState};
 });
