...
export async function configuredUniswapLpApiTvl(protocol:"v3"|"v4",pool:string,fetcher:typeof fetch=fetch):Promise<TvlResult>{
  ...
  try{
    const response=await fetcher(endpoint,{method:"POST",headers:{"content-type":"application/json",accept:"application/json","x-api-key":apiKey},body:JSON.stringify({protocol:protocol.toUpperCase(),chainId:robinhoodMainnet.chainId,poolReferences:[{poolReferenceIdentifier:pool.toLowerCase()}]})});
